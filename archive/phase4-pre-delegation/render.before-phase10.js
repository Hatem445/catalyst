...
const Renderer = {
  ...
  home() {
    ...
    return hero + stats + learningTracks + weeklyFocus + quickActions + weakBox + `<div class="section-label">الوحدات الدراسية</div><div class="units-grid">${unitsHtml}</div>`;
  },
  ...
  quiz(S) {
    ...
    const q = QUIZ[S.idx];
    const levelColor = q.level === 'سهل' ? 'var(--neon-green)' : q.level === 'متوسط' ? 'var(--neon-amber)' : 'var(--neon-red)';
    ...
    return `
      ...
      <div class="glass-card quiz-card">
        <div class="quiz-level" style="color:${levelColor};border-color:${levelColor}66">المستوى: ${q.level || 'متوسط'}</div>
