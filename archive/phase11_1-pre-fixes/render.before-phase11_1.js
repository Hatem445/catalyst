...
const MindMap = {
  ...
  reactionScene(type) {
    if (type === 'alkene_addition') {
      return `
        <svg class="rxn-svg" viewBox="0 0 340 140" xmlns="http://www.w3.org/2000/svg">
          <text x="170" y="16" text-anchor="middle" class="rxn-label">إضافة على رابطة مزدوجة</text>
          <circle cx="88" cy="68" r="14" class="rxn-atom c"/>
          <circle cx="150" cy="68" r="14" class="rxn-atom c"/>
          <line x1="102" y1="64" x2="136" y2="64" class="rxn-bond solid"/>
          <line x1="102" y1="73" x2="136" y2="73" class="rxn-bond pi break"/>
          <circle cx="250" cy="60" r="12" class="rxn-atom br rxn-move"/>
          <line x1="164" y1="68" x2="236" y2="61" class="rxn-bond dashed form"/>
          <text x="170" y="126" text-anchor="middle" class="rxn-note">تنكسر π أولاً ثم تتكون رابطة σ جديدة</text>
        </svg>`;
    }
    if (type === 'alcohol_oxidation') {
      return `
        <svg class="rxn-svg" viewBox="0 0 340 140" xmlns="http://www.w3.org/2000/svg">
          <text x="170" y="16" text-anchor="middle" class="rxn-label">أكسدة كحول أولي</text>
          <text x="68" y="70" class="rxn-formula">CH₃CH₂OH</text>
          <text x="152" y="70" class="rxn-arrow">→</text>
          <text x="176" y="70" class="rxn-formula">CH₃CHO</text>
          <text x="262" y="70" class="rxn-arrow">→</text>
          <text x="284" y="70" class="rxn-formula">CH₃COOH</text>
          <rect x="116" y="38" width="84" height="16" rx="6" class="rxn-cond pulse"/>
          <text x="158" y="50" text-anchor="middle" class="rxn-cond-text">K₂Cr₂O₇ / H₂SO₄</text>
          <text x="170" y="124" text-anchor="middle" class="rxn-note">التحكم في الشروط يمنع الأكسدة الزائدة</text>
        </svg>`;
    }
    if (type === 'esterification') {
      return `
        <svg class="rxn-svg" viewBox="0 0 340 140" xmlns="http://www.w3.org/2000/svg">
          <text x="170" y="16" text-anchor="middle" class="rxn-label">تفاعل الاسترة (عكسي)</text>
          <text x="58" y="72" class="rxn-formula">CH₃COOH</text>
          <text x="130" y="72" class="rxn-plus">+</text>
          <text x="150" y="72" class="rxn-formula">C₂H₅OH</text>
          <text x="226" y="72" class="rxn-arrow">⇌</text>
          <text x="250" y="72" class="rxn-formula">CH₃COOC₂H₅</text>
          <circle cx="224" cy="94" r="4" class="rxn-drop drift"/>
          <circle cx="236" cy="104" r="3" class="rxn-drop drift" style="animation-delay:.5s"/>
          <text x="170" y="124" text-anchor="middle" class="rxn-note">إزالة الماء تدفع الاتزان للناتج</text>
        </svg>`;
    }
    if (type === 'substitution') {
      return `
        <svg class="rxn-svg" viewBox="0 0 340 140" xmlns="http://www.w3.org/2000/svg">
          <text x="170" y="16" text-anchor="middle" class="rxn-label">إحلال في الألكانات</text>
          <text x="78" y="70" class="rxn-formula">CH₄ + Cl₂</text>
          <text x="162" y="70" class="rxn-arrow">→</text>
          <text x="188" y="70" class="rxn-formula">CH₃Cl + HCl</text>
          <polygon points="160,40 168,40 164,52" class="rxn-light blink"/>
          <text x="164" y="36" text-anchor="middle" class="rxn-cond-text">UV</text>
          <line x1="140" y1="78" x2="188" y2="78" class="rxn-bond dashed"/>
          <text x="170" y="124" text-anchor="middle" class="rxn-note">الضوء يبدأ تفاعل الجذور الحرة</text>
        </svg>`;
    }
    if (type === 'bond_switch') {
      return `
        <svg class="rxn-svg" viewBox="0 0 340 140" xmlns="http://www.w3.org/2000/svg">
          <text x="170" y="16" text-anchor="middle" class="rxn-label">كسر رابطة وتكوين أخرى</text>
          <circle cx="78" cy="70" r="11" class="rxn-atom c"/>
          <circle cx="118" cy="70" r="11" class="rxn-atom c"/>
          <line x1="89" y1="70" x2="107" y2="70" class="rxn-bond pi break"/>
          <circle cx="198" cy="58" r="10" class="rxn-atom h"/>
          <circle cx="240" cy="82" r="10" class="rxn-atom br"/>
          <line x1="126" y1="68" x2="188" y2="60" class="rxn-bond dashed form"/>
          <line x1="126" y1="74" x2="228" y2="82" class="rxn-bond dashed form" style="animation-delay:.4s"/>
          <text x="170" y="124" text-anchor="middle" class="rxn-note">الفكرة الأساسية: الروابط الأضعف تتغير أولاً</text>
        </svg>`;
    }
    return '';
  },

  reactionCard({ id, title, type }) {
    return `
...
const Renderer = {
  ...
  organic() {
    ...
    const animations = [
      { id: 'rxn_anim_1', title: 'تفاعل إضافة على الكين', type: 'alkene_addition' },
      { id: 'rxn_anim_2', title: 'أكسدة الكحول الأولي', type: 'alcohol_oxidation' },
      { id: 'rxn_anim_3', title: 'تكوين الإستر (استرة)', type: 'esterification' },
      { id: 'rxn_anim_4', title: 'مثال إحلال (كلورة الألكان)', type: 'substitution' },
      { id: 'rxn_anim_5', title: 'كسر رابطة/تكوين أخرى', type: 'bond_switch' },
    ];
    ...
    return `
      ...
      ${animations.map(a => MindMap.reactionCard(a)).join('')}

      <div class="glass-card fade-in" style="padding:12px;margin-top:10px">
        <button class="home-quick-btn" style="width:100%" data-action="tab" data-tab="periodic">فتح الجدول الدوري التفاعلي</button>
      </div>
    `;
  },
