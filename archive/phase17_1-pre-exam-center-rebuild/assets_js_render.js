...
const Renderer = {
  ...
  home(adaptiveHome = null) {
    ...
    const quickActions = `
      <div class="home-quick-grid fade-in fade-in-delay-2">
        ...
        <button class="glass-card home-quick-btn" data-action="open-exam-center">مركز الامتحانات</button>
  ...
  examCenter(model = {}) {
    const cards = model.cards || ExamCenter.getCards();
    const chips = model.chips || ExamCenter.getTopicChips();
    const summary = model.analyticsSummary || { attempted: 0, correct: 0, accuracy: 0 };
    const questions = model.filteredQuestions || [];
    const activeTopic = model.activeTopic || 'all';
    const mistakes = model.commonMistakes || [];

    const loadingBlock = model.loading
      ? '<div class="glass-card laws-empty">جارٍ تحميل بنك الامتحانات...</div>'
      : '';
    const errorBlock = model.error
      ? `<div class="glass-card laws-empty">${model.error}</div>`
      : '';

    return `
      <div class="section-label">Exam Center</div>
      <div class="glass-card exam-center-head fade-in">
        <div class="exam-center-title">مركز الامتحانات</div>
        <div class="exam-center-subtitle">تدريب امتحاني موجه حسب الموضوع والمهارة مع مراجعة الأخطاء.</div>
        <div class="exam-center-stats">
          <div class="exam-stat-box">
            <div class="exam-stat-value">${model.totalQuestions || 0}</div>
            <div class="exam-stat-label">أسئلة متاحة</div>
          </div>
          <div class="exam-stat-box">
            <div class="exam-stat-value">${summary.attempted || 0}</div>
            <div class="exam-stat-label">محاولات</div>
          </div>
          <div class="exam-stat-box">
            <div class="exam-stat-value">${summary.accuracy || 0}%</div>
            <div class="exam-stat-label">الدقة</div>
          </div>
        </div>
      </div>
      ...
      <div class="exam-center-grid fade-in">
        ${cards.map(card => `
          <button class="exam-card-btn" data-action="${card.action}" data-section="${card.section}">
      ...
      <div class="exam-chip-row fade-in">
        ${chips.map(chip => `
          <button class="exam-chip ${activeTopic === chip.key ? 'active' : ''}" data-action="exam-set-topic" data-topic="${chip.key}">
            ${chip.label}
          </button>
        `).join('')}
      </div>
      ...
      <div class="exam-list fade-in">
        ${questions.map(q => `
          <article class="exam-question-row">
            <div class="exam-question-meta">
              <span class="exam-badge">${q.topic}</span>
        ...
        `).join('')}
        ${questions.length === 0 && !model.loading ? '<div class="glass-card laws-empty">لا توجد أسئلة مطابقة لهذا الفلتر.</div>' : ''}
      </div>

      <div class="glass-card exam-center-head fade-in" style="margin-top:10px">
        <div class="exam-center-title" style="font-size:14px">أخطائي الشائعة</div>
        <div class="exam-list">
          ${mistakes.map(m => `<div class="exam-question-row"><div class="exam-question-text">سؤال ${m.questionId} • ${m.topic} • تكررت ${m.count} مرة</div></div>`).join('')}
          ${mistakes.length === 0 ? '<div class="laws-empty">لا توجد أخطاء مسجلة بعد.</div>' : ''}
        </div>
      </div>
    `;
  },
  ...
  questionPlayer(vm) {
    if (!vm) {
      return '<div class="glass-card laws-empty">تعذر فتح السؤال المطلوب.</div>';
    }
    const canConfirm = vm.selectedIndex >= 0 && !vm.confirmed;
    const feedback = vm.confirmed
      ? `<div class="qp-feedback ${vm.isCorrect ? 'ok' : 'bad'}">${vm.isCorrect ? 'إجابة صحيحة ✅' : 'إجابة غير صحيحة، راجع المثال والقانون 👇'}</div>`
      : '';
    const workedExample = vm.confirmed && vm.workedExample
      ? `
        <div class="qp-example">
          <div class="qp-example-title">${vm.workedExample.title || 'مثال محلول'}</div>
          <div class="qp-example-summary">${vm.workedExample.summary || ''}</div>
          <div class="qp-example-steps">
            ${(vm.workedExample.steps || []).map((step, idx) => `<div class="qp-example-step">${idx + 1}) ${step}</div>`).join('')}
          </div>
        </div>
      `
      : '';
    ...
    return `
      <div class="qp-wrap">
        <div class="glass-card qp-head fade-in">
          <div class="qp-title">سؤال تدريبي</div>
          <div class="qp-meta">
        ...
        <article class="glass-card qp-question-card fade-in">
          <div class="qp-stem">${vm.question}</div>
          <div class="qp-choice-list">
            ${vm.choices.map(choice => `
              <button class="qp-choice-btn ${choice.status}" data-action="exam-select-choice" data-choice-index="${choice.index}" ${vm.confirmed ? 'disabled' : ''}>
                ${choice.text}
              </button>
            `).join('')}
          </div>
          ...
          <div class="qp-actions">
            <button class="qp-confirm-btn" data-action="exam-confirm-answer" ${canConfirm ? '' : 'disabled'}>تأكيد الإجابة</button>
            ...
            <button class="qp-back-btn" data-action="open-exam-center">العودة إلى مركز الامتحانات</button>
          </div>
        </article>
