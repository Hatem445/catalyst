...
import { EXAM_CENTER_PUBLIC_ENABLED } from './state.js';
...
const Search = {
  ...
  async hydrateExamEntries() {
    if (!EXAM_CENTER_PUBLIC_ENABLED) return;
  ...
  query(q) {
    ...
    this.index.forEach(entry => {
      if (!EXAM_CENTER_PUBLIC_ENABLED && (entry.kind === 'exam-question' || entry.kind === 'worked-example')) return;
