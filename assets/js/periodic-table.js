...
const ORBITAL_CONFIG_MAX_Z = 36;

const SHELL_LABELS = ['الأول', 'الثاني', 'الثالث', 'الرابع', 'الخامس', 'السادس', 'السابع'];

const SUPER_NUM = { '0': '⁰', '1': '¹', '2': '²', '3': '³', '4': '⁴', '5': '⁵', '6': '⁶', '7': '⁷', '8': '⁸', '9': '⁹' };

const CONFIG_EXCEPTIONS = {
  24: [{ from: '4s', to: '3d', count: 1 }],
  29: [{ from: '4s', to: '3d', count: 1 }],
  41: [{ from: '5s', to: '4d', count: 1 }],
  42: [{ from: '5s', to: '4d', count: 1 }],
  44: [{ from: '5s', to: '4d', count: 1 }],
  45: [{ from: '5s', to: '4d', count: 1 }],
  46: [{ from: '5s', to: '4d', count: 2 }],
  47: [{ from: '5s', to: '4d', count: 1 }],
  57: [{ from: '4f', to: '5d', count: 1 }],
  58: [{ from: '4f', to: '5d', count: 1 }],
  64: [{ from: '4f', to: '5d', count: 1 }],
  78: [{ from: '6s', to: '5d', count: 1 }],
  79: [{ from: '6s', to: '5d', count: 1 }],
  89: [{ from: '5f', to: '6d', count: 1 }],
  90: [{ from: '5f', to: '6d', count: 2 }],
  91: [{ from: '5f', to: '6d', count: 1 }],
  92: [{ from: '5f', to: '6d', count: 1 }],
  93: [{ from: '5f', to: '6d', count: 1 }],
  96: [{ from: '5f', to: '6d', count: 1 }],
};

function toSuper(value) {
  return String(value).split('').map(ch => SUPER_NUM[ch] || ch).join('');
}

function buildOccupancy(z) {
  let remaining = Math.max(0, Number(z) || 0);
  const occ = ORBITAL_FILL_ORDER.map(o => ({ ...o, key: `${o.n}${o.sub}`, e: 0 }));
  for (let i = 0; i < occ.length && remaining > 0; i += 1) {
    const take = Math.min(occ[i].cap, remaining);
    occ[i].e = take;
    remaining -= take;
  }

  const adjust = CONFIG_EXCEPTIONS[z] || [];
  adjust.forEach(rule => {
    const from = occ.find(o => o.key === rule.from);
    const to = occ.find(o => o.key === rule.to);
    if (!from || !to) return;
    const move = Math.min(rule.count, from.e, to.cap - to.e);
    if (move > 0) {
      from.e -= move;
      to.e += move;
    }
  ...
  return occ.filter(o => o.e > 0);
}

function occupancyToShells(occ) {
  const shells = [0, 0, 0, 0, 0, 0, 0];
  occ.forEach(o => {
    if (o.n >= 1 && o.n <= 7) shells[o.n - 1] += o.e;
  });
  while (shells.length && shells[shells.length - 1] === 0) shells.pop();
  return shells;
}

function configurationText(occ, z) {
  if (z > ORBITAL_CONFIG_MAX_Z) return 'غير متاح حالياً';
  return occ.map(o => `${o.key}${toSuper(o.e)}`).join(' ');
}

function renderShellRows(shells) {
  if (!shells.length) return '<div class="periodic-shell-row">غير متاح حالياً</div>';
  return shells.map((count, i) => `<div class="periodic-shell-row"><span>الغلاف ${SHELL_LABELS[i] || i + 1}</span><strong>${count}</strong></div>`).join('');
}

function renderBohrAtom(el) {
  const shells = Array.isArray(el.shellDistribution) ? el.shellDistribution : [];
  if (!shells.length) {
    return '<div class="periodic-atom-empty">غير متاح حالياً</div>';
  }

  const center = 160;
  const shellGroups = shells.map((count, idx) => {
    const radius = 34 + (idx * 19);
    const duration = 8 + (idx * 2.5);
    const reverseClass = idx % 2 ? ' atom-orbit-reverse' : '';
    const electronSize = count >= 18 ? 1.9 : count >= 10 ? 2.2 : 2.6;
    const electrons = Array.from({ length: count }).map((_, eIdx) => {
      const angle = (Math.PI * 2 * eIdx) / count;
      const x = center + Math.cos(angle) * radius;
      const y = center + Math.sin(angle) * radius;
      return `<circle class="atom-electron" cx="${x.toFixed(2)}" cy="${y.toFixed(2)}" r="${electronSize}"></circle>`;
    }).join('');

    return `<g class="atom-orbit${reverseClass}" style="--orbit-dur:${duration}s">
      <circle class="atom-shell-ring" cx="${center}" cy="${center}" r="${radius}"></circle>
      ${electrons}
    </g>`;
  }).join('');

  return `<div class="periodic-atom-wrap">
    <svg class="periodic-atom-svg" viewBox="0 0 320 320" aria-label="نموذج بور للعنصر">
      ${shellGroups}
      <g class="atom-nucleus">
        <circle cx="${center}" cy="${center}" r="26"></circle>
        <text x="${center}" y="${center - 2}" text-anchor="middle" class="atom-nucleus-symbol">${el.s}</text>
        <text x="${center}" y="${center + 13}" text-anchor="middle" class="atom-nucleus-z">Z=${el.z}</text>
      </g>
    </svg>
  </div>`;
}

function groupMeaning(el) {
  if (!el || !el.g) return 'غير متاح حالياً';
  if (el.g === 1) return 'المجموعة 1: غالبًا إلكترون تكافؤ واحد (عدا الهيدروجين بسلوك خاص).';
  if (el.g === 2) return 'المجموعة 2: غالبًا إلكترونا تكافؤ، وتميل لفقدهما في التفاعل.';
  if (el.g >= 13 && el.g <= 18) return `المجموعة ${el.g}: عناصر رئيسية تتدرج خواصها عبر الجدول داخل نفس العمود.`;
  if (el.g >= 3 && el.g <= 12) return `المجموعة ${el.g}: عنصر انتقالي، وإلكترونات d تؤثر في خواصه الكيميائية.`;
  return `المجموعة ${el.g}: عناصر لها نمط خواص متشابه داخل نفس العمود.`;
}

function periodMeaning(el) {
  if (!el || !el.p) return 'غير متاح حالياً';
  return `الدورة ${el.p}: تشير لعدد مستويات الطاقة الرئيسية المشغولة في الذرة.`;
}

function configurationMeaning(el) {
  if (!el) return 'غير متاح حالياً';
  const outer = Array.isArray(el.shellDistribution) && el.shellDistribution.length
    ? el.shellDistribution[el.shellDistribution.length - 1]
    : null;
  const shellInfo = outer === null ? '' : ` إلكترونات الغلاف الخارجي هنا تقريبًا: ${outer}.`;
  if (!el.electronConfig || el.electronConfig === 'غير متاح حالياً') {
    return `صيغة المدارات التفصيلية غير متاحة حاليًا، لكن توزيع الأغلفة يوضح ترتيب الإلكترونات.${shellInfo}`;
  }
  return `التوزيع الإلكتروني يوضح ترتيب الإلكترونات داخل المدارات من الأقل طاقة إلى الأعلى.${shellInfo}`;
}

function renderEducationPanel(el) {
  return `<div class="periodic-edu-panel">
    <div class="periodic-edu-title">شرح تعليمي سريع</div>
    <div class="periodic-edu-grid">
      <div class="periodic-edu-card">
        <div class="periodic-edu-label">معنى المجموعة</div>
        <div class="periodic-edu-text">${groupMeaning(el)}</div>
      </div>
      <div class="periodic-edu-card">
        <div class="periodic-edu-label">معنى الدورة</div>
        <div class="periodic-edu-text">${periodMeaning(el)}</div>
      </div>
      <div class="periodic-edu-card periodic-edu-wide">
        <div class="periodic-edu-label">معنى التوزيع الإلكتروني</div>
        <div class="periodic-edu-text">${configurationMeaning(el)}</div>
      </div>
    </div>
  </div>`;
}
...
const ELEMENTS = ELEMENTS_RAW
  ...
  .map(el => {
    ...
    const occ = buildOccupancy(el.z);
    const shellDistribution = occupancyToShells(occ);
    return {
      ...el,
      category: cat.ar,
      categoryKey: cat.key,
      mass: el.m || 'غير متاح',
      shellDistribution,
      shellText: shellDistribution.length ? shellDistribution.join('، ') : 'غير متاح حالياً',
      electronCount: el.z,
      electronConfig: configurationText(occ, el.z),
    };
  });
...
function renderDetail(selectedZ) {
  ...
  return `<div class="periodic-detail">
    ...
    <div class="periodic-detail-meta-chips">
      <span class="periodic-meta-chip">Z = ${el.z}</span>
      <span class="periodic-meta-chip">المجموعة ${el.g || 'غير متاح'}</span>
      <span class="periodic-meta-chip">الدورة ${el.p || 'غير متاح'}</span>
      <span class="periodic-meta-chip">${el.category || 'غير متاح'}</span>
    </div>
    <div class="periodic-detail-grid">
      ...
      <div><span>عدد الإلكترونات:</span><strong>${el.electronCount || 'غير متاح حالياً'}</strong></div>
      <div><span>مستويات الطاقة:</span><strong class="chem-ltr">${el.shellText || 'غير متاح حالياً'}</strong></div>
      <div class="periodic-detail-wide"><span>التوزيع الإلكتروني:</span><strong class="chem-ltr">${el.electronConfig || 'غير متاح حالياً'}</strong></div>
      <div class="periodic-detail-wide"><span>توزيع الأغلفة:</span><div class="periodic-shell-list">${renderShellRows(el.shellDistribution || [])}</div></div>
      <div class="periodic-detail-wide">
        <span>نموذج بور المتحرك:</span>
        ${renderBohrAtom(el)}
      </div>
    </div>
    ${renderEducationPanel(el)}
  </div>`;
}
