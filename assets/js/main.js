...
function updateInstallButtonVisibility() {
  ...
  if (!btn) return;
  btn.style.display = '';
  btn.classList.toggle('is-ready', Boolean(deferredInstallPrompt));
  btn.title = deferredInstallPrompt
    ? 'تثبيت التطبيق'
    : 'يمكن تثبيت التطبيق من قائمة المتصفح.';
  const label = btn.querySelector('.install-text');
  if (label) {
    label.textContent = window.innerWidth <= 420 ? 'تثبيت' : 'تثبيت التطبيق';
  }
}

async function handleInstallApp() {
  if (!deferredInstallPrompt) {
    toast('يمكن تثبيت التطبيق من قائمة المتصفح.');
    return;
  }
...
function updateSelectedSequenceUI(card) {
  const activity = card.dataset.activity || '';
  const selected = parseSelection(card.dataset.selected);
  card.querySelectorAll('[data-action="organic-seq-toggle"]').forEach(btn => {
    const active = selected.includes(btn.dataset.stepKey || '');
    btn.classList.toggle('is-selected', active);
  });
  const selectedText = card.querySelector(`[data-organic-selected="${activity}"]`);
  if (selectedText) {
    selectedText.textContent = selected.length
      ? `الترتيب الحالي: ${selected.join(' → ')}`
      : 'لم يتم اختيار خطوات بعد.';
  }
}
...
function handleDelegatedClick(event) {
  ...
  switch (action) {
    ...
    case 'restart-rxn-anim':
      ...
      break;
    case 'organic-seq-toggle': {
      const card = target.closest('.organic-activity-card');
      if (!card) break;
      const key = target.dataset.stepKey || '';
      const selected = parseSelection(card.dataset.selected);
      if (!key) break;
      if (selected.includes(key)) {
        card.dataset.selected = selected.filter(k => k !== key).join(',');
      } else {
        selected.push(key);
        card.dataset.selected = selected.join(',');
      }
      updateSelectedSequenceUI(card);
      break;
    }
    case 'organic-seq-check': {
      const card = target.closest('.organic-activity-card');
      if (!card) break;
      const activity = card.dataset.activity || '';
      const selected = parseSelection(card.dataset.selected);
      const correct = parseSelection(card.dataset.correct);
      const ok = selected.length === correct.length && selected.every((step, idx) => step === correct[idx]);
      const feedback = card.querySelector(`[data-organic-feedback="${activity}"]`);
      if (feedback) {
        feedback.textContent = ok
          ? 'ترتيب صحيح. ممتاز.'
          : 'الترتيب غير صحيح. حاول ترتيب الخطوات: إضافة ماء ثم تكوين الكحول ثم أكسدة أولى ثم أكسدة ثانية.';
        feedback.classList.toggle('is-success', ok);
        feedback.classList.toggle('is-error', !ok);
      }
      break;
    }
    case 'organic-seq-reset': {
      const card = target.closest('.organic-activity-card');
      if (!card) break;
      card.dataset.selected = '';
      updateSelectedSequenceUI(card);
      const feedback = card.querySelector(`[data-organic-feedback="${card.dataset.activity || ''}"]`);
      if (feedback) {
        feedback.textContent = 'اختر الترتيب ثم اضغط تحقق.';
        feedback.classList.remove('is-success', 'is-error');
      }
      break;
    }
    case 'organic-reagent-select': {
      const card = target.closest('.organic-activity-card');
      if (!card) break;
      const key = target.dataset.choiceKey || '';
      card.dataset.choice = key;
      card.querySelectorAll('[data-action="organic-reagent-select"]').forEach(btn => {
        btn.classList.toggle('is-selected', btn.dataset.choiceKey === key);
      });
      break;
    }
    case 'organic-reagent-check': {
      const card = target.closest('.organic-activity-card');
      if (!card) break;
      const activity = card.dataset.activity || '';
      const choice = card.dataset.choice || '';
      const ok = choice && choice === (card.dataset.correct || '');
      const feedback = card.querySelector(`[data-organic-feedback="${activity}"]`);
      if (feedback) {
        feedback.textContent = ok
          ? 'إجابة صحيحة: إضافة الماء تحتاج وسطًا حمضيًا H2O/H2SO4.'
          : 'إجابة غير دقيقة. التحويل إلى الإيثانول يتم بإضافة الماء في وسط حمضي.';
        feedback.classList.toggle('is-success', Boolean(ok));
        feedback.classList.toggle('is-error', !ok);
      }
      break;
    }
    case 'organic-reagent-reset': {
      const card = target.closest('.organic-activity-card');
      if (!card) break;
      card.dataset.choice = '';
      card.querySelectorAll('[data-action="organic-reagent-select"]').forEach(btn => {
      ...
      const choicesWrap = card.querySelector('.organic-grid-2');
      if (choicesWrap) {
        const buttons = Array.from(choicesWrap.querySelectorAll('[data-action="organic-reagent-select"]'));
        for (let i = buttons.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          if (i !== j) {
            const bi = buttons[i];
            const bj = buttons[j];
            const ni = bi.cloneNode(true);
            const nj = bj.cloneNode(true);
            bi.replaceWith(nj);
            bj.replaceWith(ni);
            buttons[i] = nj;
            buttons[j] = ni;
      ...
      const feedback = card.querySelector(`[data-organic-feedback="${card.dataset.activity || ''}"]`);
      if (feedback) {
        feedback.textContent = 'اختر كاشفًا ثم أكد الإجابة.';
        feedback.classList.remove('is-success', 'is-error');
      }
      break;
    }
